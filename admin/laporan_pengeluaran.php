...
<h3 style="margin-top: 0;"><b>Laporan Pengeluaran</b></h3>
<hr />
...
if (empty($tgl_awal) or empty($tgl_akhir)) { // Cek jika tgl_awal atau tgl_akhir kosong, maka :            
   ...
   $url_cetak = "cetakpengeluaran.php";
   $label = "Semua Data Transaksi";
} else { // Jika terisi            
   // Buat query untuk menampilkan data transaksi sesuai periode tanggal            
   $query = "SELECT * FROM pengeluaran WHERE (tanggal BETWEEN '" . $tgl_awal . "' AND '" . $tgl_akhir . "')";
   $url_cetak = "cetakpengeluaran.php?tgl_awal=" . $tgl_awal . "&tgl_akhir=" . $tgl_akhir . "&filter=true";
      ...
      <tbody>
         ...
         if ($row > 0) { // Jika jumlah data lebih dari 0 (Berarti jika data ada)                 
            while ($data = mysqli_fetch_array($sql)) { // Ambil semua data dari hasil eksekusi $sql                   
               ...
               echo "<td>" . $data['kd_pengeluaran'] . "</td>";
